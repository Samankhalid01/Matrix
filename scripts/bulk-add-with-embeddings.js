...
async function generateEmbedding(productData, retries = 3) {
...
// Main function
export default async function runBulkAdd() {
  ...
  for (let i = 0; i < products.length; i++) {
    ...
    try {
      ...
      successCount++;
      console.log(`✅ [${progress}%] Added: ${product.product_name.padEnd(30)} (ID: ${data.id}) WITH embedding`);
      
    } catch (error) {
      failCount++;
      failedProducts.push({ name: product.product_name, error: error.message });
  ...
  return { successCount, failCount, embeddingCount };
}

// Allow direct execution
runBulkAdd().catch(console.error);
